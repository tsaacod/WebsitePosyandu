...
<x-layout>
    ...
    <form action="{{ route('imunisasi.store') }}" method="POST" class="mt-4">
        @csrf
        <div class="mb-4">
            ...
            <select name="bayi_id" id="bayi_id" class="border border-gray-300 rounded-md px-4 py-2 w-full">
                @foreach ($bayi as $b)
                    <option value="{{ $b->id }}">{{ $b->nama_bayi }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
            <label for="jenis_imunisasi" class="block text-gray-700">Jenis Imunisasi</label>
            <input type="text" name="jenis_imunisasi" id="jenis_imunisasi" class="border border-gray-300 rounded-md px-4 py-2 w-full">
        </div>
        <div class="mb-4">
            <label for="tanggal_imunisasi" class="block text-gray-700">Tanggal Imunisasi</label>
            <input type="date" name="tanggal_imunisasi" id="tanggal_imunisasi" class="border border-gray-300 rounded-md px-4 py-2 w-full">
        </div>
        <div class="mb-4">
            <label for="keterangan" class="block text-gray-700">Keterangan</label>
            <textarea name="keterangan" id="keterangan" class="border border-gray-300 rounded-md px-4 py-2 w-full"></textarea>
        </div>
        <div class="mt-6">
            <button type="submit" class="bg-[#297F4C] text-white px-4 py-2 rounded-md hover:bg-green-600 transition duration-200">Simpan</button>
        </div>
    </form>
</x-layout>
